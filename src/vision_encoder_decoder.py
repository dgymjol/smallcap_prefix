...
class SmallCap(PreTrainedModel):
    ...
    def forward(
        self,
        pixel_values=None,
        decoder_input_ids=None, # <- prefix + lables
        decoder_attention_mask=None,
        encoder_outputs=None,
        past_key_values=None,
        decoder_inputs_embeds=None,  # <- it2 features
        labels=None,
        use_cache=None,
        ...
        **kwargs,
        ...
        if decoder_inputs_embeds is not None:
            decoder_prefix_embeds = self.decoder.transformer.wte(decoder_input_ids) # ([64, 140, 768])
            if labels is not None : # train
                for i in range(decoder_prefix_embeds.size(0)):
                    # insert_idx = ((labels[i] != -100).nonzero()[0] - 1)
                    # decoder_prefix_embeds[i][insert_idx] = decoder_inputs_embeds[i]
                    decoder_prefix_embeds[i][:77] = decoder_inputs_embeds[i]
                    
            else: # infer
                for i in range(decoder_prefix_embeds.size(0)):
                    # decoder_prefix_embeds[i][-2] = decoder_inputs_embeds.squeeze(0)     
                    
                    # for 1 token 
                    # decoder_prefix_embeds[i][:77] = decoder_inputs_embeds[i]
                    # for 77 token
                    decoder_prefix_embeds[i][:77] = decoder_inputs_embeds[0]
                    
            decoder_input_ids = None
        else:
            decoder_prefix_embeds = None
        ...
        decoder_outputs = self.decoder(
            ...
            inputs_embeds=decoder_prefix_embeds,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            use_cache=use_cache,
    ...
    def prepare_inputs_for_generation(
        ...
        decoder_inputs_embeds = encoder_outputs.hidden_states
