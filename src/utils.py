...
def prep_strings(text, tokenizer, template=None, retrieved_caps=None, k=None, is_test=False, max_length=None):
    ...
    prefix_ids = [tokenizer.eos_token_id] * 77 + prefix_ids
...
class TrainDataset(Dataset):
    def __init__(self, df, features_path, i2t_features_path, tokenizer, rag=False, template_path=None, k=None, max_caption_length=25):
        self.df = df
        self.tokenizer = tokenizer
        self.features = h5py.File(features_path, 'r')
        self.prefixes = h5py.File(i2t_features_path, 'r')
    ...
    def __getitem__(self, idx):
        ...
        encoding = {"encoder_outputs": torch.tensor(encoder_outputs), 
                    ...
                    "decoder_inputs_embeds": self.prefixes[self.df['cocoid'][idx]][()]}
