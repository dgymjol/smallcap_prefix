...
def evaluate_rag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    if args.features_path is not None:
        ...
        prefixes = h5py.File(args.i2t_features_path, 'r')
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        if args.features_path is not None:
            ...
            encoder_outputs = BaseModelOutput(last_hidden_state=encoder_last_hidden_state.to(args.device),
                                              hidden_states=torch.FloatTensor([prefixes[image_id][()]]).to(args.device))
            with torch.no_grad():
        ...
        # for 1 token
        # pred = tokenizer.decode(pred[0][(pred[0] == 220).nonzero()[0] + 1:])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--i2t_features_path", type=str, default='features/val.hdf5', help="H5 file with cached input image features")
